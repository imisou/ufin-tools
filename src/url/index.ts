...
type Url = (arg: string | number, url?: string) => string;

export const url: Url = (function () {
  function _t() {
    return new RegExp(
...
export const getUrlQuery = (urlstr?: string): UrlQueryData => {
  ...
  return url('?', urlstr) as UrlQueryData;
};
 ...
 */
export const getUrlQueryByName = (name?: string, urlstr?: string): string | undefined => {
  // url 地址
  urlstr = !urlstr ? window.location.href : urlstr;
  // 查询条件
  name = name ? name.trim() : '';

  return url(`?${name}`, urlstr);
};
